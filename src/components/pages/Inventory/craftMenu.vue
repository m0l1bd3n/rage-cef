<template>
    ...
    <div class="title-container border-radius-top">
      <div class="title-container_text">
        КРАФТ
      </div>
    </div>
    <div class="craft-container_items-filter">
      <div class="search-block">
        <svg class="icon" width="1.85vh" height="1.85vh" viewBox="0 0 20 20" fill="none"
      ...
      <div class="items-container">
        <div class="items-container_item" v-for="item in itemsCraftPick" :key="item.text">
          <p>{{ item.text }}</p>
        </div>
      </div>
    </div>
    <div class="craft-container_items-list">
      <div class="items-container">
        <div class="items-container_item" v-for="item in itemsCraftList" :key="item.text">
          <div class="items-container_item-image">
            <img alt="IMG" src="../../../assets/img/inventory/test_shapka.png">
          </div>
          <p class="items-container_item-title">{{ item.text }}</p>
        </div>
      </div>
    </div>
</template>
...
export default {
  name: "craftMenu",
  props: {
    itemsCraftPick: String,
    itemsCraftList: String,
  }
}
</script>
